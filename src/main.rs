...
use runtime_module::{
    ...
    is_module_enabled_in_content, verify_modules_exist, ModuleRegistry, ModuleStatus,
};
...
fn verify_modules(modules: &[String]) {
    ...
    if !verify_modules_exist(modules, &registry) {
...
fn is_module_enabled(module_name: &str) -> bool {
    ...
    is_module_enabled_in_content(module_name, &content)
}
...
fn get_active_modules() -> Vec<String> {
    ...
    get_active_modules_from_content(&content)
}

// Create or update the runtime-modules.nix file with the specified modules
fn generate_modules_file(modules: &[String]) {
    let registry = load_modules_registry();
    let module_paths: Vec<(String, String)> = modules
        .iter()
        .filter_map(|module| {
            if let Some(path) = get_module_path(module, &registry) {
    ...
    if json_output {
        let status_list: Vec<_> = modules
            ...
            .map(|module| {
                ...
                let path = get_module_path(module, &registry).unwrap_or_default();
